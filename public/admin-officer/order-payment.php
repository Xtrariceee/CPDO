...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    notify_user((int)$application['landlord_id'], $applicationId, 'Order of Payment Issued', 'An Order of Payment (OP No. ' . $order['op_number'] . ') has been issued for your application. Please proceed to payment.');
...
if ($application) {
    $logStmt = db()->prepare(
        "SELECT al.created_at, al.details, u.first_name, u.last_name
         FROM audit_logs al
         LEFT JOIN users u ON u.id = al.user_id
         WHERE al.action = 'ORDER_OF_PAYMENT_GENERATED'
           AND al.entity_type = 'payment_orders'
           AND al.entity_id IN (SELECT id FROM payment_orders WHERE application_id = ?)
    ...
    <div class="col-lg-8">
        <?php if ($application): ?>
            <section class="gov-card p-4 mb-4" data-sensitive>
                ...
                <?php else: ?>
                    <div class="text-center py-4 text-secondary">
                        ...
                        <p class="mb-0 small">No Order of Payment generated yet.<br>
                        Click <strong>Generate Payment Slip</strong> below to create one.</p>
                    </div>
                <?php endif; ?>
            ...
            <section class="gov-card p-4">
                ...
                <?php if ($opLogs): ?>
                    <div class="table-responsive">
                        <table class="table table-sm align-middle mb-0">
                            <thead><tr><th>Date &amp; Time</th><th>OP Number</th><th>Generated By</th></tr></thead>
                            <tbody>
                                <?php foreach ($opLogs as $log): ?>
                                    <?php
                                    $details  = json_decode($log['details'] ?? '{}', true);
                                    $opNumber = $details['op_number'] ?? '—';
                                    $genBy    = trim(($log['first_name'] ?? '') . ' ' . ($log['last_name'] ?? '')) ?: 'System';
                                    $ts       = $log['created_at'] ? date('M d, Y · h:i A', strtotime($log['created_at'])) : '—';
                                    ?>
                                    <tr>
                                        <td class="small text-secondary"><?= e($ts) ?></td>
                ...
                <?php else: ?>
                    <p class="text-secondary small mb-0">No generation history yet.</p>
                <?php endif; ?>
            </section>
        <?php else: ?>
