...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($applicationId > 0 && isset($statusMap[$status])) {
        ...
        db()->prepare('UPDATE applications SET status = ?, phase_status = ?, current_process = ? WHERE id = ?')
           ->execute([$status, $phaseStatus, $process, $applicationId]);
                                ...
                                <form class="d-flex gap-2" method="post">
                                    ...
                                    <select class="form-select form-select-sm" name="status">
                                        <?php foreach (array_keys($statusMap) as $s): ?>
                                            <option value="<?= e($s) ?>" <?= $app['status'] === $s ? 'selected' : '' ?>><?= e(workflow_status_label($s)) ?></option>
                                        <?php endforeach; ?>
                                    </select>
