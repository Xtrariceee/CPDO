...
$uploads    = db()->query('SELECT cu.*, CONCAT_WS(" ", u.first_name, u.middle_name, u.last_name) AS landlord_name, u.email FROM compliance_uploads cu JOIN users u ON u.id=cu.landlord_id ORDER BY cu.created_at DESC')->fetchAll();
$selected   = null;
foreach ($uploads as $upload) {
    if ($selectedId && (int)$upload['id'] === $selectedId) { $selected = $upload; break; }
}
