...
$pendingDesignations = (int)db()->query('SELECT COUNT(*) FROM staff_designation_requests WHERE status = "PENDING"')->fetchColumn();
$logs = db()->query(
...
<div class="layout-grid">
    ...
    <section>
        ...
        <div class="metric-grid mb-4" style="grid-template-columns:repeat(5,minmax(0,1fr));">
            ...
            <article class="metric-card" style="border-left:4px solid var(--cpdo-red);">
                <span>Designation Requests</span>
                <strong><?= $pendingDesignations ?></strong>
                <?php if ($pendingDesignations > 0): ?>
                    <a href="../users/" class="small fw-bold d-block mt-1" style="color:var(--cpdo-red);">Review →</a>
                <?php endif; ?>
            </article>
        </div>
        <div class="row g-4">
                        ...
                        <table class="table align-middle">
                            ...
                            <?php foreach (array_slice($applications, 0, 8) as $app): ?>
                                <tr>
                                    ...
                                    <td><span class="status-pill status-<?= e($app['status']) ?>"><?= e(workflow_status_label($app['status'])) ?></span></td>
                                    <td>P<?= (int)$app['current_process'] ?></td>
                                </tr>
                            <?php endforeach; ?>
                            <?php if (!$applications): ?>
                                <tr><td colspan="4" class="text-secondary">No applications yet.</td></tr>
                            <?php endif; ?>
                            </tbody>
                        </table>
                    </div>
