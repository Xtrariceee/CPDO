...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $update = db()->prepare('UPDATE requirement_documents SET evaluation_status=?,officer_notes=?,evaluated_by=?,evaluated_at=NOW() WHERE id=?');
    foreach ($docs->fetchAll() as $doc) {
        $status = $_POST['doc_'.$doc['id']] ?? 'PENDING';
        if (in_array($status, ['PENDING','PASSED','FAILED'], true)) {
            $update->execute([$status, trim($_POST['notes_'.$doc['id']] ?? ''), (int)$user['id'], (int)$doc['id']]);
        }
    }
    $pending = db()->prepare('SELECT COUNT(*) FROM requirement_documents WHERE application_id=? AND evaluation_status!="PASSED"');
    $pending->execute([$applicationId]);
    if ((int)$pending->fetchColumn() === 0) {
        create_payment_order_if_missing($application, (int)$user['id']);
        notify_user((int)$application['landlord_id'], $applicationId, 'Documents Approved', 'All your submitted documents have passed evaluation. An Order of Payment has been generated — please proceed to payment.');
        ...
        notify_user((int)$application['landlord_id'], $applicationId, 'Document Evaluation Updated', 'Your application documents have been reviewed. Please log in to check the evaluation results and address any failed items.');
...
<div class="d-flex align-items-center gap-3 mb-4">
    ...
    <div><h1 class="h3 mb-0">Pre-Evaluation</h1></div>
</div>
<div class="row g-4">
    <div class="col-lg-3">
            ...
            <?php if ($applications): ?>
                <div class="d-grid gap-1">
                    <?php foreach ($applications as $row): ?>
                        ...
                        <a class="app-list-item <?= $isActive?'app-list-item--active':'' ?>" href="pre-evaluation.php?id=<?= (int)$row['id'] ?>">
            ...
            <div class="gov-card p-4 mb-4">
                <div class="d-flex justify-content-between align-items-start gap-3 flex-wrap">
                    <div>
                        ...
                        <p class="text-secondary small mb-0"><?= e($application['registry_number']) ?> · <?= e($application['landlord_name']) ?></p>
                    </div>
                    <span class="status-pill status-<?= e($application['status']) ?>"><?= e(workflow_status_label($application['phase_status'])) ?></span>
                </div>
                <div class="row g-3 mt-3">
                    <div class="col-4"><div class="eval-stat-card eval-stat-card--neutral"><span><?= $uploadedDocs ?>/<?= $totalDocs ?></span><small>Uploaded</small></div></div>
                    <div class="col-4"><div class="eval-stat-card eval-stat-card--pass"><span><?= $passedDocs ?></span><small>Passed</small></div></div>
                    <div class="col-4"><div class="eval-stat-card eval-stat-card--fail"><span><?= $failedDocs ?></span><small>Failed</small></div></div>
                </div>
            </div>
            <form class="gov-card p-4" method="post" id="eval-form">
                ...
                <div class="d-flex justify-content-between align-items-center mb-3">
                    ...
                    <div class="eval-actions">
                        <button type="button" class="eval-bulk-btn eval-bulk-btn--pass" id="mark-all-pass">Pass All</button>
                        <button type="button" class="eval-bulk-btn eval-bulk-btn--reset" id="mark-all-pending">Reset All</button>
                    </div>
                </div>
                <?php $currentGroup = ''; foreach ($documents as $doc): ?>
                    <?php if ($currentGroup !== $doc['group_name']): $currentGroup = $doc['group_name']; ?>
                        <div class="eval-group-header"><span class="upload-office-label">Office:</span><strong><?= e($currentGroup) ?></strong></div>
                    <?php endif; ?>
                        ...
                        <div>
                            ...
                            <div class="eval-doc-meta">
                                ...
                                <?php if ($hasFile): ?>
                                    ...
                                    <button type="button" class="preview-link" data-preview-btn
                                        ...
                                        data-preview-meta="<?= e($doc['group_name']) ?>">Preview Document</button>
                                <?php else: ?>
                                    <span class="badge text-bg-danger">No file uploaded</span>
                                <?php endif; ?>
                            </div>
                            <div class="eval-doc-notes">
                                <input class="form-control form-control-sm" name="notes_<?= (int)$doc['id'] ?>" value="<?= e($doc['officer_notes'] ?? '') ?>" placeholder="Officer notes (optional)">
                            </div>
                        </div>
                        <div class="d-flex align-items-start pt-1">
                            <select class="eval-select <?= $doc['evaluation_status']==='PASSED'?'status-passed':($doc['evaluation_status']==='FAILED'?'status-failed':'status-pending') ?>" name="doc_<?= (int)$doc['id'] ?>" data-eval-select>
                ...
                <div class="d-flex gap-3 mt-4 align-items-center flex-wrap">
                    ...
                    <span class="text-secondary small">
                        <?= $passedDocs === $totalDocs && $totalDocs > 0 ? 'All documents passed - saving will generate an Order of Payment.' : ($totalDocs - $passedDocs) . ' document(s) not yet passed.' ?>
                    </span>
                </div>
            </form>
        <?php else: ?>
            <div class="gov-card p-5"><div class="empty-state"><div class="empty-state-icon">DOC</div><h3>No applications to evaluate</h3><p>Submitted applications will appear here once landlords complete their document uploads.</p></div></div>
        <?php endif; ?>
    </div>
</div>
<style>
...
.eval-stat-card{border-radius:var(--radius-md);padding:12px 14px;text-align:center;border:1px solid var(--cpdo-border);}
.eval-stat-card span{display:block;font-size:1.6rem;font-weight:900;line-height:1;}
.eval-stat-card small{display:block;font-size:var(--text-xs);font-weight:700;text-transform:uppercase;letter-spacing:.06em;margin-top:4px;}
...
.eval-group-header{display:flex;align-items:baseline;gap:8px;margin:24px 0 0;padding:8px 12px;background:var(--cpdo-light);border-radius:var(--radius-sm);border-left:4px solid var(--cpdo-blue);}
</style>
<script>
(function(){
    ...
    function setAll(v){selects.forEach(function(s){s.value=v;s.dispatchEvent(new Event('change'));});}
